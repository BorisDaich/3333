...
describe('Catalogs Management Component', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([{ path: 'catalogs', component: CatalogsComponent }]),
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.catalogs?.[0]).toEqual(expect.objectContaining({ id: 'ABC' }));
  });

  describe('trackId', () => {
    it('Should forward to catalogsService', () => {
      const entity = { id: 'ABC' };
      jest.spyOn(service, 'getCatalogsIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getCatalogsIdentifier).toHaveBeenCalledWith(entity);
